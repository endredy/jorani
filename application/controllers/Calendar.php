...
class Calendar extends CI_Controller {
    ...
    public function tabularPartial($id=-1, $month=0, $year=0, $children=TRUE, $displayTypes=TRUE) {
        if (($this->config->item('public_calendar') === TRUE) && (!$this->session->userdata('logged_in'))) {
            ...
            $data['tabular'] = $this->leaves_model->tabular($id, $month, $year, $children, $statuses, $this->input->get('types'));
            $data['entity'] = $id;
            $data['month'] = $month;
            $data['year'] = $year;
            $data['children'] = $children;
            ...
            $data['mode'] = 'connected';
            $statuses = $this->input->get('statuses');
            $data['tabular'] = $this->leaves_model->tabular($id, $month, $year, $children, $statuses, TRUE, $this->input->get('types'));
            $data['entity'] = $id;
            $data['month'] = $month;
            $data['year'] = $year;
            $data['children'] = $children;
